...
function POMDPs.states(pomdp::SingleOCPOMDP)
        ...
        for v in V
            for y in Y
                for v_ped in V_ped
                    ...
                    push!(space, SingleOCState(collision_checker(ego, ped, pomdp.ego_type, pomdp.ped_type), ego, ped))
                end
            end
        end
    end
    ...
    push!(space, TERMINAL_STATE)
    return space
end

function POMDPs.stateindex(pomdp::SingleOCPOMDP, s::SingleOCState)
    ...
    x_0 = 5
    x_ind = ceil(Int, (x - x_0)/pomdp.pos_res) + 1
    v_ind = ceil(Int, v/pomdp.vel_res) + 1
    y_ind = ceil(Int, (y - y_0)/pomdp.pos_res) + 1

    if !off_the_grid(pomdp, s.ped)
        v_ped_ind = ceil(Int, v_ped/pomdp.vel_res) + 1
...
function n_states(pomdp::SingleOCPOMDP)
    ...
    lw = env.params.lane_width
    cl = env.params.crosswalk_length
    size_x = floor(Int, (rl/2 + 2*cw -5)/pomdp.pos_res) + 1
    size_v = floor(Int, env.params.speed_limit/pomdp.vel_res) + 1
    size_y = floor(Int, cl/2/pomdp.pos_res) + 1
    size_v_ped = floor(Int, env.params.ped_max_speed/pomdp.vel_res) + 1
    return size_x*size_v*(size_y*size_v_ped + 1) + 1
end
...
function POMDPs.obsindex(pomdp::SingleOCPOMDP, o::SingleOCObs)
