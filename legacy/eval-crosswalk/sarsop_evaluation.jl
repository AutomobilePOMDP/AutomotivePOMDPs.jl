...
@everywhere begin
    using AutoViz, Reel, POMDPs, StatsBase, POMDPToolbox, SARSOP, QMDP, AutomotiveDrivingModels, JLD, Parameters, Distributions, GridInterpolations
    ...
    sarsop_policy = load_policy(pomdp, "/scratch/boutonm/crosswalk$(abs(pomdp.collision_cost)).policy")

    sensor = POMDPSensor(pomdp = pomdp, sensor = SimpleSensor(0.5, 0.5))
    policy = QMDPEval(env, pomdp, sarsop_policy);
    up = MixedUpdater(pomdp, config.sim_dt);
    a0 = 0.
    o0 = Dict{Int64, OCObs}()
    ego0 = initial_ego(env, config.rng)
    b0 = Dict{Int64, OCDistribution}()
    b0[-1] = initialstate(policy.pomdp, ego0.state)
    update_freq = 5
...
result_file = "../results_sarsop.csv"
...
res_str = string(scenario, ", ", pomdp.goal_reward, ", ", pomdp.action_cost, ", ", pomdp.collision_cost, ", ", pomdp.p_birth, ", ",
                 ...
                 c_rate, ", ", suc_rate, ", ", to_rate, ", ", avg_time, "\n")
write(f, res_str)
close(f)

using DataFrames
convergence_file="conv_sarsop_$p_birth$collision_cost.csv"
